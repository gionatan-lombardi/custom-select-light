function CustomSelect(elem, panelClass, optionClass) {

    'use strict';

    // Private vars
    var $select = $(elem+' select');
    var $opener = $(elem+' .cstSelLabel');
    var $panel, $cstOption, optionsData, searchTimeout, searchString = "";

    // Optional arguments
    if ( typeof panelClass === 'undefined' ) var panelClass = 'custom-select-panel'
    if ( typeof optionClass === 'undefined' ) var optionClass = 'form-control'
    ...
    function customSelectCreation () {
        ...
        for (var i = 0;  i < optionsData.value.length; i++) {
            optionsMarkup += '<div class="cstSelOption '+ optionClass +'" data-val="'+ optionsData.value[i] +'"><span>'+ optionsData.text[i] +'</span></div>';
        };

        // Creates the custom select HTML markup
        var customSelectMarkup = '<div class="cstSelPanel '+ panelClass +'">'+ optionsMarkup +'</div>';

        // Writes the custom select HTML markup in the DOM
        ...
        $panel = $(elem+" .cstSelPanel");
        $cstOption = $(elem+" .cstSelOption");
    ...
    function setFocus ($elem, elemIndex, isMoving, isAnimated) {
        ...
        if ( isMoving === true ) { 
            ...
            $panel.stop().animate({scrollTop:gotoPos}, duration);
        }
    }
    ...
    function searchInPanel (e) {
        ...
        return { $elem: $elem, elemIndex: elemIndex }
    }
